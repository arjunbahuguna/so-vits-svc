...
def train_and_evaluate(rank, epoch, hps, nets, optims, schedulers, scaler, loaders, logger, writers):
    ...
    for batch_idx, items in enumerate(train_loader):
        f0, spec, y, spk, lengths, uv, volume = items
        ...
        with autocast(enabled=hps.train.fp16_run, dtype=half_type):
            ...
            (z, z_p, m_p, logs_p, m_q, logs_q), pred_lf0, norm_lf0, lf0 = net_g(f0, uv, spec, g=g, c_lengths=lengths,
...
def evaluate(hps, generator, eval_loader, writer_eval):
    ...
    with torch.no_grad():
        for batch_idx, items in enumerate(eval_loader):
            f0, spec, y, spk, _, uv, volume = items
            g = spk[:1].cuda(0)
            ...
            mel = spec_to_mel_torch(
                ...
                hps.data.mel_fmin,
                hps.data.mel_fmax)
            y_hat, _ = generator.module.infer(f0, uv, g=g, vol=volume)
